...
Function getGroupName(areaName)
    If Not IsNumeric(areaName) Or areaName <= 0 Then
	    getGroupName = ""
	Else
	    Dim rs,squareName
		Set rs = Server.CreateObject("ADODB.Recordset")
		rs.Open "Select * From baidu_group Where areaName = " & areaName,conn,3,1
...
Function list()
    ...
    Dim rs
    Set rs = Server.CreateObject("ADODB.Recordset")
    rs.Open "Select * From baidu_square Order By areaName",conn,3,1
    Do While Not rs.BOF And Not rs.EOF
    	strToPrint = strToPrint & "        <tr>" & VBCrlf
    	strToPrint = strToPrint & "          <td align=""center"">" & rs("areaName") & "</td>" & VBCrlf
    	strToPrint = strToPrint & "          <td>" & rs("squareName") & "</td>" & VBCrlf
    	strToPrint = strToPrint & "          <td align=""center"">[<a href=""?action=edit&squareNameOld=" & rs("squareName") & """>修改</a>] [<a href=""?action=delete&squareNameOld=" & rs("squareName") & """ onClick=""return confirm('确定要删除该组和\r该组内所有用户？');"">删除</a>]</td>" & VBCrlf
    	rs.MoveNext
    Loop
    rs.Close
    Set rs = Nothing
...
Sub saveData()
	...
	If rs.RecordCount > 0 Then
	    Response.Write("<div align=""center"">该小区已经存在。<br>")
		Response.Write("<input name=""back"" type=""button"" value="" 返 回 "" onClick=""history.back();""></div>")
		rs.Close
		Set rs = Nothing
		Exit Sub
...
%>
		    <table width="100%" border="0" cellpadding="3" cellspacing="0" bgcolor="#FFFFFF">
              ...
              <tr> 
                <td width="60">&nbsp;</td>
                  <td>业务小区名称： 
                    <input name="squareName" type="text" id="squareName" size="16" maxlength="16" value="<% = squareName %>" selfValue="用户级别名称">
                    业务区域 ： 
                    <% = getList(1,"baidu_area","","areaName","areaName","业务区域") %>
                  </td>
              </tr>
              <tr> 
...
[<a href="system_area.asp">业务区域</a>] [<a href="system_square.asp">业务小区</a>] [<a href="system_del1.asp">客户删除</a>] [<a href="system_del2.asp">记录删除</a>] [<a href="system_del3.asp">拜访删除</a>] </div></td>
      ...
      <table width="100%" align="center" border="1" cellpadding="3" cellspacing="0" bordercolor="#DCDCDC" bordercolordark="#FFFFFF">
        <tr> 
          <td width="120" align="center" bgcolor="menu">业务区域名称</td>
          <td align="center" bgcolor="menu">业务小区名称</td>
