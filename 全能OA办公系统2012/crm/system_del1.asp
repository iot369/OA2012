...
Function list()
    ...
    Dim rs
    Set rs = Server.CreateObject("ADODB.Recordset")
    rs.Open "Select * From baidu_client Order By cId",conn,3,1
    Do While Not rs.BOF And Not rs.EOF
    	strToPrint = strToPrint & "        <tr>" & VBCrlf
    	strToPrint = strToPrint & "          <td align=""center"">" & rs("cId") & "</td>" & VBCrlf
    	strToPrint = strToPrint & "          <td>" & rs("cCompany") & "</td>" & VBCrlf
    	strToPrint = strToPrint & "          <td align=""center"">[<a href=""?action=delete&lNameOld=" & rs("cCompany") & """ onClick=""return confirm('确定要删除该\r客户吗？');"">删除</a>]</td>" & VBCrlf
    	rs.MoveNext
    Loop
    rs.Close
    Set rs = Nothing
...
Sub deleteData()
	...
	Dim rs
	Set rs = Server.CreateObject("ADODB.Recordset")
	rs.Open "Select * From baidu_client Where cCompany = '" & lNameOld & "'",conn,3,2
	If rs.RecordCount > 0 Then
	    lIdOld = rs("cId")
		rs.Delete
		rs.Update
...
[<a href="system_area.asp">业务区域</a>] [<a href="system_square.asp">业务小区</a>] [<a href="system_del1.asp">客户删除</a>] [<a href="system_del2.asp">记录删除</a>] [<a href="system_del3.asp">拜访删除</a>] </div></td>
      ...
      <table width="100%" align="center" border="1" cellpadding="3" cellspacing="0" bordercolor="#DCDCDC" bordercolordark="#FFFFFF">
        <tr> 
          ...
          <td align="center" bgcolor="menu">客户公司名称</td>
