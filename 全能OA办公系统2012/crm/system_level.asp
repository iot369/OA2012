...
Function list()
    ...
    Dim rs
    Set rs = Server.CreateObject("ADODB.Recordset")
    rs.Open "Select * From baidu_level Order By lId",conn,3,1
    Do While Not rs.BOF And Not rs.EOF
    	strToPrint = strToPrint & "        <tr>" & VBCrlf
    	strToPrint = strToPrint & "          <td align=""center"">" & rs("lId") & "</td>" & VBCrlf
    	strToPrint = strToPrint & "          <td>" & rs("lName") & "</td>" & VBCrlf
    	strToPrint = strToPrint & "          <td align=""center"">[<a href=""?action=edit&lNameOld=" & rs("lName") & """>修改</a>] [<a href=""?action=delete&lNameOld=" & rs("lName") & """ onClick=""return confirm('确定要删除该级别和该\r级别下的所有用户？');"">删除</a>]</td>" & VBCrlf
    	rs.MoveNext
    Loop
    rs.Close
    Set rs = Nothing
...
%>
		    <table width="100%" border="0" cellpadding="3" cellspacing="0" bgcolor="#FFFFFF">
              <form name="levelForm" action="<% = strAction %>" method="post" onSubmit="return checkInput('levelForm');">
			  <tr> 
                <td width="60">&nbsp;</td>
                <td><% = strOut %>
                    <% If action = "edit" Then %><input name="lNameOld" type="hidden" id="lNameOld" value="<% = lNameOld %>"><% End If %></td>
              </tr>
              <tr> 
                <td width="60">&nbsp;</td>
                <td>用户级别名称： 
                  <input name="lName" type="text" id="lName" size="16" maxlength="16" value="<% = lName %>" selfValue="用户级别名称">
                  级别编号： 
                  <input name="lId" type="text" id="lId" size="2" maxlength="2" value="<% = lId %>" selfValue="级别编号">
...
%>
  ...
  <tr>
    <td valign="top" bgcolor="#FFFFFF" style="padding: 10px;"><table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td height="40"><div align="center">[<a href="system_level.asp">用户级别</a>] [<a href="system_group.asp">用 
            户 
            组</a> ] 
              [<a href="system_clientsType.asp">客户等级</a>] [<a href="system_clientsTrade.asp">行业类型</a>] 
            [<a href="system_recordsType.asp">拜访类型</a>]<br> 
              [<a href="system_area.asp">业务区域</a>] 
            [<a href="system_square.asp">业务小区</a>] 
            [<a href="system_del1.asp">客户删除</a>] 
            [<a href="system_del2.asp">记录删除</a>] 
            [<a href="system_del3.asp">拜访删除</a>]
          </div></td>
        </tr>
      </table>
      <table width="100%" align="center" border="1" cellpadding="3" cellspacing="0" bordercolor="#DCDCDC" bordercolordark="#FFFFFF">
        <tr> 
          ...
          <td align="center" bgcolor="menu">用户级别名称</td>
