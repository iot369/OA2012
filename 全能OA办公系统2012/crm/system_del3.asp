...
Function getLevelName(lId)
	...
	Else
	    Dim rs,lName
		Set rs = Server.CreateObject("ADODB.Recordset")
		rs.Open "Select * From 人 Where rId = " & rId,conn,3,1
		If rs.RecordCount <> 1 Then
		    lName = ""
		Else
		    lName = rs("lName")
...
Function list()
    ...
    Dim rs
    Set rs = Server.CreateObject("ADODB.Recordset")
    rs.Open "Select * From baidu_recordsPlan Order By rId",conn,3,1
    Do While Not rs.BOF And Not rs.EOF
...
Sub deleteData()
	...
	Dim rs
	Set rs = Server.CreateObject("ADODB.Recordset")
	rs.Open "Select * From baidu_recordsPlan Where rType = '" & lNameOld & "'",conn,3,2
	If rs.RecordCount > 0 Then
	    lIdOld = rs("rId")
		rs.Delete
		rs.Update
...
[<a href="system_area.asp">业务区域</a>] [<a href="system_square.asp">业务小区</a>] [<a href="system_del1.asp">客户删除</a>] [<a href="system_del2.asp">记录删除</a>] [<a href="system_del3.asp">拜访删除</a>] </div></td>
      ...
      <table width="100%" align="center" border="1" cellpadding="3" cellspacing="0" bordercolor="#DCDCDC" bordercolordark="#FFFFFF">
        <tr> 
          ...
          <td align="center" bgcolor="menu">拜访客户内容</td>
