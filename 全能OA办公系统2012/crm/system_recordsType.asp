...
Function list()
    ...
    '	strToPrint = strToPrint & "          <td align=""center"">" & rs("lId") & "</td>" & VBCrlf
    	...
    	strToPrint = strToPrint & "          <td align=""center"">[<a href=""?action=edit&recordsTypeOld=" & rs("recordsType") & """>修改</a>] [<a href=""?action=delete&recordsTypeOld=" & rs("recordsType") & """ onClick=""return confirm('确定要删除该拜访类型和该\r类型下的所有拜访资料？');"">删除</a>]</td>" & VBCrlf
    	rs.MoveNext
    Loop
    rs.Close
    Set rs = Nothing
...
Sub saveData()
    ...
    Dim rs
	Set rs = Server.CreateObject("ADODB.Recordset")
	rs.Open "Select  * From baidu_recordsType Where recordsType = '" & recordsType & "'",conn,3,2
	If rs.RecordCount > 0 Then
	    Response.Write("<div align=""center"">该拜访类型已经存在。<br>")
		Response.Write("<input name=""back"" type=""button"" value="" 返 回 "" onClick=""history.back();""></div>")
		rs.Close
		Set rs = Nothing
		Exit Sub
...
Sub restore()
    Dim recordsTypeOld,recordsType
	recordsTypeOld = Trim(Request.Form("recordsTypeOld"))
	recordsType = Trim(Request.Form("recordsType"))
	If recordsTypeOld = "" Or recordsType = "" Then
	    Response.Write("<div align=""center"">提交的数据不完整，请返回重新填写。<br>")
		Response.Write("<input name=""back"" type=""button"" value="" 返 回 "" onClick=""history.back();""></div>")
		Exit Sub
	End If
    Dim rs
	Set rs = Server.CreateObject("ADODB.Recordset")
	rs.Open "Select  * From baidu_recordsType Where recordsType <> '" & recordsTypeOld & "'",conn,3,1
	Do While Not rs.BOF And Not rs.EOF
	...
	rs.Open "Select * From baidu_recordsType Where recordsType = '" & recordsTypeOld & "'",conn,3,2
	If rs.RecordCount = 1 Then
	    recordsTypeOld = rs("recordsType")
		rs("recordsType") = recordsType
		rs.Update
		If recordsTypeOld <> recordsType Then
		    Dim rss
			Set rss = Server.CreateObject("ADODB.Recordset")
			rss.Open "Select * From baidu_records Where rType = '" & recordsTypeOld & "'",conn,3,2
			Do While Not rss.BOF And Not rss.EOF
			    rss("cType") = recordsType
				rss.Update
				rss.MoveNext
			Loop
			rss.Close
...
Sub deleteData()
    Dim recordsTypeOld,lIdOld
	recordsTypeOld = Trim(Request("recordsTypeOld"))
	If recordsTypeOld = "" Then
	    Response.Write("<div align=""center"">提交的数据不完整，请返回重新填写。<br>")
		Response.Write("<input name=""back"" type=""button"" value="" 返 回 "" onClick=""history.back();""></div>")
		Exit Sub
	End If
	...
	If recordsTypeOld <> "" Then
	    rs.Open "Select * From baidu_records Where rType = '" & recordsTypeOld & "'",conn,3,2
		Do While Not rs.BOF And Not rs.EOF
		    rs.Delete
			rs.Update
			rs.MoveNext
...
Sub addOrEdit()
    Dim recordsType,recordsTypeOld,strOut,strAction
	If action = "edit" Then
	    Dim rs
		recordsTypeOld = Trim(Request("recordsTypeOld"))
		Set rs = Server.CreateObject("ADODB.Recordset")
...
%>
		    <table width="100%" border="0" cellpadding="3" cellspacing="0" bgcolor="#FFFFFF">
              <form name="recordsTypeForm" action="<% = strAction %>" method="post" onSubmit="return checkInput('recordsTypeForm');">
			  <tr> 
                <td width="60">&nbsp;</td>
                <td><% = strOut %>
                    <% If action = "edit" Then %><input name="recordsTypeOld" type="hidden" id="recordsTypeOld" value="<% = recordsTypeOld %>"><% End If %></td>
              </tr>
              <tr> 
                <td width="60">&nbsp;</td>
                  <td>拜访类型名称： 
                    <input name="recordsType" type="text" id="recordsType" size="16" maxlength="16" value="<% = recordsType %>" selfValue="拜访类别名称">
                  </td>
              </tr>
              <tr> 
...
[<a href="system_area.asp">业务区域</a>] [<a href="system_square.asp">业务小区</a>] [<a href="system_del1.asp">客户删除</a>] [<a href="system_del2.asp">记录删除</a>] [<a href="system_del3.asp">拜访删除</a>] </div></td>
      ...
      <table width="100%" align="center" border="1" cellpadding="3" cellspacing="0" bordercolor="#DCDCDC" bordercolordark="#FFFFFF">
        <tr> 
          <td align="center" bgcolor="menu">拜访类型名称</td>
